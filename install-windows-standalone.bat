...
powershell -ExecutionPolicy Bypass -Command "& {$WshShell = New-Object -comObject WScript.Shell; $Desktop = [System.Environment]::GetFolderPath('Desktop'); $ShortcutPath = '$Desktop\Nástroje OP JAK.lnk'; $Shortcut = $WshShell.CreateShortcut($ShortcutPath); $Shortcut.TargetPath = '%INSTALL_DIR%\start-app.bat'; $Shortcut.WorkingDirectory = '%INSTALL_DIR%'; $Shortcut.IconLocation = '%INSTALL_DIR%\src\electron\assets\icon.ico'; $Shortcut.Description = 'Nástroje pro zpracování dokumentace OP JAK'; $Shortcut.Save(); Write-Host '✅ Zástupce vytvořen na ploše' -ForegroundColor Green}"
if errorlevel 1 (
